...
namespace GarconWinApp.Models
{
    public class MenuItem: IGarcon
    {
        ...
        {
            get { return string.Format("{0} {1} : {2} - ({3}) {4} min. cook time", IsChefRecommendation ? "*" : "", MenuItemType.ToString(), Name, GetItemPriceWTax(ItemPrice).ToString(ApplicationSettings.CurrencyFormat), PrepTimeInMinutes); }
